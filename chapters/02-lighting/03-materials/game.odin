...
game_update :: proc(game: ^Game, dt: f64) {
    ...
    //color: glsl.vec3 = {math.sin_f32(f32(glfw.GetTime()) * 1.5), math.sin_f32(f32(glfw.GetTime()) * 1.0), math.sin_f32(f32(glfw.GetTime()) * 0.5)}
    //game.light.diffuse = color * glsl.vec3({0.5, 0.5, 0.5})
    //game.light.ambient = game.light.diffuse * glsl.vec3({0.2, 0.2, 0.2})
}
