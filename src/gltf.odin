...
gltf_load ::proc(meshes: ^map[string]Mesh, name: string, path: string) {
    ...
    log.debugf("found %d positions, %d normals, %d texture coordinates", pos_accessor.count, normal_accessor.count, texcoord_accessor.count)
    ...
    for i in 0..< num_verts {
        ...
        // Read vertex normals
        if !cgltf.accessor_read_float(pos_accessor, i, &verts[i * 8 + 3], 3) {
            log.errorf("cgltf vertex normal reading failed.")
            os.exit(1)
        }
        /*
        // Read vertex texture coordinates
        if i < 10 && !cgltf.accessor_read_float(pos_accessor, i, &verts[i * 8 + 3 + 3], 2) {
